...
# 5. 自定义阈值 20 MB，报告保存为 big_files.txt
...
> ⚠️ 请在测试目录中验证后再对重要资料目录执行写操作。

1. **先预览再执行**  
   `--rename-images` 和 `--clean-empty` 会修改文件系统，建议先用 `--dry-run` 预览结果。

2. **图片重命名不可自动撤销**  
   重命名采用两阶段策略避免文件名冲突，但操作完成后无法通过脚本还原，请提前备份重要图片。

3. **不会删除根目录**  
   `--clean-empty` 只删除子目录中的空文件夹，传入的根目录本身不会被删除。

4. **大文件报告写入位置**  
   报告默认保存在**被扫描目录内**（如 `test/report.txt`），可通过 `--report` 自定义文件名。

5. **权限问题**  
   若目录或文件被占用、无读写权限，部分操作可能跳过并在控制台提示，不会中断整个程序。

6. **递归范围**  
   所有功能均会**递归处理子目录**，请确认 `-p` 指向的目录范围是否符合预期。

7. **组合执行顺序**  
   使用 `--all` 或多参数组合时，执行顺序为：统计 → 重命名图片 → 大文件报告 → 清理空文件夹。